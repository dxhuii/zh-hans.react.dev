...
`useActionState` 是一个 React Hook，可以让你使用 [Actions](/reference/react/useTransition#functions-called-in-starttransition-are-called-actions) 来更新具有副作用的状态。
...
在组件的顶层调用 `useActionState` 来创建一个表示 Action 结果的状态。
...
1. 当前的 state。第一次渲染期间，该值为传入的 `initialState` 参数值。在 `dispatchAction` 被调用后该值会变为 `reducerAction` 的返回值。
2. 一个 `dispatchAction` 函数，可以在 [Actions](/reference/react/useTransition#functions-called-in-starttransition-are-called-actions) 中调用。
3. 一个 `isPending` 标识，用于表明当前 Actions 是否在 pending 状态。
...
在组件的顶层调用 `useActionState` 来为 Action 的结果创建一个状态。
...
1. 该表单的 <CodeStep step={1}>当前 state</CodeStep>，初始值是传入的参数。
2. 该表单的  <CodeStep step={2}>action 调度函数</CodeStep>，用于让你触发 `reducerAction`。
3. 一个 <CodeStep step={3}>pending state</CodeStep>，用于指示 Action 是否还在执行中。
