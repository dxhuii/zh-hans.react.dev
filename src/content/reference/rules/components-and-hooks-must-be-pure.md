...
“渲染”指的是计算你的用户界面（UI）下一个版本应该呈现的样子。渲染完成后，React 会用这个新的计算结果与你 UI 上一个版本所用的计算结果进行比较，然后 React 仅将必要的变化（也就是用户实际看到的部分）提交到 [DOM](https://developer.mozilla.org/en-US/docs/Web/API/Document_Object_Model) 来应用更改。最终，[Effects](/learn/synchronizing-with-effects) 会被“清空”（意思是一直运行完所有的 Effect 为止）。更多详细信息请查看 [Render](/learn/render-and-commit) 和 [Commit and Effect Hooks](/reference/react/hooks#effect-hooks)。
